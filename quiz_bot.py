...
async def table_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Anyone: current league standings."""
    rows = compute_league_table()
    if not rows:
        await update.message.reply_text("No table data yet.")
        return
    lines = [
        f"{i+1}. {html.escape(r['name'])} — P{r['played']} W{r['won']} D{r['drawn']} L{r['lost']} "
        f"GD{r['gf']-r['ga']:+d} Pts{r['points']}"
        for i, r in enumerate(rows)
    ]
    await update.message.reply_text("<b>🏆 League Table</b>\n\n" + "\n".join(lines), parse_mode=ParseMode.HTML)
