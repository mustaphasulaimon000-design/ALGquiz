...
def get_db():
    ...
    if not firebase_admin._apps:
        ...
        if cred_json:
            ...
            # Falls back to GOOGLE_APPLICATION_CREDENTIALS pointing at a key file, if set instead.
...
def has_valid_lineup(info: dict) -> bool:
    ...
    return bool(lineup and len(lineup.get("starters", [])) == LINEUP_STARTERS)
...
def save_all_state():
    ...
    for chat_id, state in quiz_states.items():
        db.collection(COL_STATE).document(str(chat_id)).set({
            "questions": [list(q) for q in state.questions],
...
def save_pending():
    ...
    db.collection(COL_PENDING).document("shared").set(
        {"questions": [list(q) for q in pending_questions]}
    )
...
def load_pending():
    ...
    if doc.exists:
        pending_questions = [tuple(q) for q in (doc.to_dict() or {}).get("questions", [])]
...
async def linkplayers_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin-only. Usage: /linkplayers Team Name — links Telegram members to real website squad players."""
    if update.effective_chat.type == "private":
        await update.message.reply_text("Run this inside the group.")
        return
    if not await is_admin(update, context):
    ...
    if not tg_team_arg:
        await update.message.reply_text("Usage: /linkplayers Team Name")
        return
    ...
    if not unlinked:
        await update.message.reply_text(
            f"Everyone on {tg_team} is already linked. Use /links {tg_team} to view them."
        )
        return
...
async def link_pick_telegram_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not builder:
        await query.answer("This session expired — run /linkplayers again.", show_alert=True)
        return
...
async def link_cancel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text("Player linking cancelled. Run /linkplayers again to retry.")


async def links_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Usage: /links Team Name — shows the permanent Telegram-person <-> squad-player links for a team."""
    if update.effective_chat.type == "private":
        await update.message.reply_text("Run this inside the group.")
        return
    team_arg = update.message.text.partition(" ")[2].strip()
    if not team_arg:
        await update.message.reply_text("Usage: /links Team Name")
        return

    chat_id = update.effective_chat.id
    matching = [link for link in get_player_links(chat_id) if link.get("team_name", "").lower() == team_arg.lower()]
    if not matching:
        await update.message.reply_text(f"No links yet for '{team_arg}'. Use /linkplayers {team_arg} to set them up.")
        return
...
async def lineup_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    members = info["members"]
    needed = LINEUP_STARTERS + LINEUP_SUBS
    if len(members) < needed:
        await update.message.reply_text(
            f"{team_name} needs at least {needed} members before a lineup can be set (has {len(members)})."
        )
        return
    ...
    await update.message.reply_text(
        f"<b>📋 Pick {LINEUP_STARTERS} starters for {html.escape(team_name)} (0/{LINEUP_STARTERS} selected)</b>",
        parse_mode=ParseMode.HTML,
        reply_markup=keyboard,
    )
...
async def lineup_toggle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    limit = LINEUP_STARTERS if stage == "starters" else LINEUP_SUBS
    ...
    if stage == "starters":
        pool = list(members.keys())
        header = f"📋 Pick {LINEUP_STARTERS} starters for {html.escape(team_name)} ({len(current)}/{LINEUP_STARTERS} selected)"
    else:
        pool = [m for m in members if m not in builder["starters"]]
        header = f"📋 Pick {LINEUP_SUBS} subs for {html.escape(team_name)} ({len(current)}/{LINEUP_SUBS} selected)"
...
async def lineup_confirm_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if builder["stage"] == "starters":
        if len(builder["starters"]) != LINEUP_STARTERS:
            await query.answer(f"Pick exactly {LINEUP_STARTERS} starters first.", show_alert=True)
            return
        builder["stage"] = "subs"
        await query.answer()
        members = get_lineup_pool(builder)
        ...
        await query.edit_message_text(
            f"<b>📋 Pick {LINEUP_SUBS} subs for {html.escape(team_name)} (0/{LINEUP_SUBS} selected)</b>",
            parse_mode=ParseMode.HTML,
            reply_markup=keyboard,
        )
        return

    if len(builder["subs"]) != LINEUP_SUBS:
        await query.answer(f"Pick exactly {LINEUP_SUBS} subs first.", show_alert=True)
        return
...
async def post_squad_lineup_pickers(chat_id: int, bot, fixture: dict) -> None:
    ...
    for tg_team_name, website_team_id in sides:
        ...
        if len(squad) < LINEUP_STARTERS + LINEUP_SUBS:
            await bot.send_message(
                chat_id,
                f"<b>⚠️ {html.escape(team_name)}'s website squad only has {len(squad)} player(s) — "
                f"needs at least {LINEUP_STARTERS + LINEUP_SUBS}.</b>",
                parse_mode=ParseMode.HTML,
            )
            continue
        ...
        await bot.send_message(
            ...
            f"Captain, pick your {LINEUP_STARTERS} starters (0/{LINEUP_STARTERS} selected)</b>",
            parse_mode=ParseMode.HTML,
            reply_markup=keyboard,
        )
...
def build_match_picker_keyboard(matches: list) -> InlineKeyboardMarkup:
    rows = []
    for m in matches[:20]:  # keep the list manageable
        label = f"{m.get('homeTeamName', '?')} vs {m.get('awayTeamName', '?')} (MD{m.get('matchday', '?')})"
...
async def schedulematch_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Pick the match to schedule:", reply_markup=build_match_picker_keyboard(matches)
    )
...
async def schedmatch_pick_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        "Now send the kickoff date & time, like: 2026-09-20 18:00"
    )
        ...
        if lineup_source == "squad":
            ...
            if not link:
                await update.message.reply_text(
                    ...
                    f"an admin needs to run /linkplayers {team_name}."
                )
                return
...
def main():
    ...
    app.add_handler(CommandHandler("linkplayers", linkplayers_cmd))
