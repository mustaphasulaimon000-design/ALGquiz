...
username_cache: dict[int, dict] = {}     # chat_id -> {username_lower: (user_id, first_name)} — built from messages seen, since bots can't look up an @username directly
...
ADMIN_USER_IDS = {8913859431, 8475867602, 868597186, 6480741267, 7059456808}   # <-- replace with your real Telegram user ID(s)
...
def fetch_all_league_matches(league_id: str = LEAGUE_ID) -> list:
    db = get_db()
    docs = db.collection(WEBSITE_MATCHES_COL).where("leagueId", "==", league_id).stream()
    return [{"id": d.id, **(d.to_dict() or {})} for d in docs]


def get_league_team_ids(league_id: str = LEAGUE_ID) -> set:
    """Every team that's actually appeared in a fixture for this league."""
    ids = set()
    for m in fetch_all_league_matches(league_id):
        if m.get("homeTeamId"):
...
def get_league_team_names(league_id: str = LEAGUE_ID) -> dict:
    """team_id -> team name, for every team in this league."""
    db = get_db()
    names = {}
    for tid in get_league_team_ids(league_id):
        doc = db.collection(WEBSITE_TEAMS_COL).document(tid).get()
        names[tid] = (doc.to_dict() or {}).get("name", "?") if doc.exists else "?"
    return names


def compute_league_table(league_id: str = LEAGUE_ID) -> list:
    """Standings sorted like the website's own sortLeagueTable: points, then goal difference,
    then goals for, then name. Prefers a per-league teamEntries doc if one exists."""
    db = get_db()
    rows = []
    for tid in get_league_team_ids(league_id):
        team_doc = db.collection(WEBSITE_TEAMS_COL).document(tid).get()
        team = team_doc.to_dict() if team_doc.exists else {}
        entry_doc = db.collection(WEBSITE_TEAM_ENTRIES_COL).document(f"{tid}_{league_id}").get()
        stats = entry_doc.to_dict() if (entry_doc and entry_doc.exists) else team
        rows.append({
            "name": team.get("name", "?"),
            "played": stats.get("played", 0) or 0,
            "won": stats.get("won", 0) or 0,
            "drawn": stats.get("drawn", 0) or 0,
            "lost": stats.get("lost", 0) or 0,
            "gf": stats.get("gf", 0) or 0,
            "ga": stats.get("ga", 0) or 0,
            "points": stats.get("points", 0) or 0,
        })
    rows.sort(key=lambda r: (-r["points"], -(r["gf"] - r["ga"]), -r["gf"], r["name"]))
    return rows


def fetch_league_players(league_id: str = LEAGUE_ID) -> list:
    db = get_db()
    players = []
    for tid in get_league_team_ids(league_id):
        docs = db.collection(WEBSITE_PLAYERS_COL).where("teamId", "==", tid).stream()
        for d in docs:
            p = {"id": d.id, **(d.to_dict() or {})}
            players.append(p)
    return players
...
async def assign_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin-only. Either reply to a member's message with /assign Team Name, or tag them
    directly: /assign @username Team Name (works once they've sent a message in this group,
    or if you tag them via Telegram's own suggestion list — either way resolves reliably)."""
    if update.effective_chat.type == "private":
        await update.message.reply_text("Run this inside the group.")
        return
    if not await is_admin(update, context):
    ...
    mention_entity = next((e for e in (message.entities or []) if e.type in ("mention", "text_mention")), None)

    if mention_entity:
        if mention_entity.type == "text_mention" and mention_entity.user:
            # Telegram embedded the full user directly — always reliable, no cache needed.
            target_user_id = mention_entity.user.id
            target_display_name = mention_entity.user.first_name
        else:
            # Plain "@username" text — bots can't resolve this directly, so check our own cache
            # of usernames seen from messages sent in this group.
            username = text[mention_entity.offset: mention_entity.offset + mention_entity.length].lstrip("@").lower()
            cached = username_cache.get(update.effective_chat.id, {}).get(username)
            if not cached:
                await message.reply_text(
                    f"Haven't seen a message from @{username} in this group yet, so I can't resolve who "
                    "that is. Ask them to send any message first, then try again — or reply to one of "
                    "their existing messages with /assign Team Name instead."
                )
                return
            target_user_id, target_display_name = cached

        before = text[:mention_entity.offset]
        after = text[mention_entity.offset + mention_entity.length:]
        team_arg = (before + after).partition(" ")[2].strip()

    elif message.reply_to_message:
        target_user_id = message.reply_to_message.from_user.id
        target_display_name = message.reply_to_message.from_user.first_name
        team_arg = text.partition(" ")[2].strip()

    else:
        await message.reply_text(
            "Usage: reply to their message with /assign Team Name, or tag them directly: "
            "/assign @username Team Name"
        )
        return

    if not team_arg:
        await message.reply_text("Missing team name.")
        return

    chat_id = update.effective_chat.id
    team_name = find_registered_team(chat_id, team_arg)
    if not team_name:
        await message.reply_text("No such team. Check /teams for the exact name.")
        return

    chat_teams = get_teams(chat_id)
    for info in chat_teams.values():
        info["members"].pop(target_user_id, None)
    chat_teams[team_name]["members"][target_user_id] = target_display_name
    save_teams()
    await message.reply_text(f"✅ {target_display_name} assigned to team {team_name}.")
...
def format_matchday_results(matchday: str, matches: list) -> str:
    lines = []
    for m in sorted(matches, key=lambda x: x.get("homeTeamName", "")):
        home, away = m.get("homeTeamName", "?"), m.get("awayTeamName", "?")
        if m.get("status") == "played":
            lines.append(f"⚽ {html.escape(home)} {m.get('homeScore', 0)} - {m.get('awayScore', 0)} {html.escape(away)}")
        else:
            lines.append(f"🕒 {html.escape(home)} vs {html.escape(away)} (upcoming)")
    return f"<b>Matchday {matchday}</b>\n\n" + "\n".join(lines)


def build_results_matchday_keyboard(matches: list) -> InlineKeyboardMarkup:
    by_matchday: dict = {}
    for m in matches:
        by_matchday.setdefault(m.get("matchday", "?"), []).append(m)
    rows = []
    for md in sorted(by_matchday, key=lambda x: (isinstance(x, str), x)):
        group = by_matchday[md]
        played = sum(1 for m in group if m.get("status") == "played")
        rows.append([InlineKeyboardButton(f"Matchday {md} ({played}/{len(group)} played)", callback_data=f"resmd:{md}")])
    return InlineKeyboardMarkup(rows)


async def results_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Anyone: browse past, present, and upcoming results by matchday."""
    matches = fetch_all_league_matches()
    if not matches:
        await update.message.reply_text("No matches found for this league.")
        return
    await update.message.reply_text(
        "📅 Pick a matchday:", reply_markup=build_results_matchday_keyboard(matches)
    )


async def resmd_pick_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    _, matchday_str = query.data.split(":", 1)
    matches = fetch_all_league_matches()
    matchday_matches = [m for m in matches if str(m.get("matchday", "?")) == matchday_str]
    if not matchday_matches:
        ...
        return
    await query.answer()
    keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("« Back to matchdays", callback_data="resmdback")]])
    await query.edit_message_text(
        format_matchday_results(matchday_str, matchday_matches), parse_mode=ParseMode.HTML, reply_markup=keyboard
    )


async def resmd_back_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    matches = fetch_all_league_matches()
    if not matches:
        await query.answer()
        await query.edit_message_text("No matches found for this league.")
        return
    await query.answer()
    await query.edit_message_text("📅 Pick a matchday:", reply_markup=build_results_matchday_keyboard(matches))


def format_league_table_pretty(rows: list) -> str:
    name_width = min(18, max(4, max(len(r["name"]) for r in rows)))
    header = f"{'#':>2} {'Team':<{name_width}} {'P':>2} {'W':>2} {'D':>2} {'L':>2} {'GD':>4} {'Pts':>4}"
    lines = [header, "-" * len(header)]
    for i, r in enumerate(rows, 1):
        name = r["name"][:name_width]
        gd = r["gf"] - r["ga"]
        lines.append(
            f"{i:>2} {name:<{name_width}} {r['played']:>2} {r['won']:>2} {r['drawn']:>2} "
            f"{r['lost']:>2} {gd:>+4} {r['points']:>4}"
        )
    return "\n".join(lines)


async def table_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Anyone: current league standings, as an aligned table."""
    rows = compute_league_table()
    if not rows:
        await update.message.reply_text("No table data yet.")
        return
    table_text = html.escape(format_league_table_pretty(rows))
    await update.message.reply_text(
        f"<b>🏆 League Table</b>\n<pre>{table_text}</pre>", parse_mode=ParseMode.HTML
    )


async def topscorers_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Anyone: top goal scorers across the league."""
    team_names = get_league_team_names()
    players = fetch_league_players()
    scorers = sorted((p for p in players if (p.get("goals") or 0) > 0), key=lambda p: p.get("goals", 0), reverse=True)[:10]
    if not scorers:
        await update.message.reply_text("No goals scored yet.")
        return
    lines = [
        f"{i+1}. {html.escape(p.get('name', '?'))} ({html.escape(team_names.get(p.get('teamId'), '?'))}) — {p.get('goals', 0)} goal(s)"
        for i, p in enumerate(scorers)
    ]
    await update.message.reply_text("<b>⚽ Top Scorers</b>\n\n" + "\n".join(lines), parse_mode=ParseMode.HTML)


async def topassists_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Anyone: top assisters across the league."""
    team_names = get_league_team_names()
    players = fetch_league_players()
    assisters = sorted((p for p in players if (p.get("assists") or 0) > 0), key=lambda p: p.get("assists", 0), reverse=True)[:10]
    if not assisters:
        await update.message.reply_text("No assists recorded yet.")
        return
    lines = [
        f"{i+1}. {html.escape(p.get('name', '?'))} ({html.escape(team_names.get(p.get('teamId'), '?'))}) — {p.get('assists', 0)} assist(s)"
        for i, p in enumerate(assisters)
    ]
    await update.message.reply_text("<b>🅰️ Top Assists</b>\n\n" + "\n".join(lines), parse_mode=ParseMode.HTML)
...
def compute_player_rank(player: dict) -> tuple:
    """Mirrors the website's getPlayerRank/getPlayerRankScore. Returns (icon, label, score)."""
    goals = (player.get("goals") or 0) + (player.get("careerGoals") or 0)
    assists = (player.get("assists") or 0) + (player.get("careerAssists") or 0)
    apps = (player.get("appearances") or 0) + (player.get("careerApps") or 0)
    score = round(goals * 3 + assists * 2 + apps * 0.5)
    for threshold, icon, label in PLAYER_RANK_TIERS:
        if score >= threshold:
            return icon, label, score
    return PLAYER_RANK_TIERS[-1][1], PLAYER_RANK_TIERS[-1][2], score


# Telegram bots can only react with emoji from a fixed platform-allowed set — crowns/gems/medals
# aren't confirmed to be in it, so each tier above maps to a safe stand-in for the reaction itself.
RANK_REACTION_EMOJI = {"Legend": "🏆", "Diamond": "🔥", "Gold": "⚡", "Silver": "👍", "Bronze": "💯", "Rookie": "👏"}


async def maybe_react_with_rank_badge(update: Update, context: ContextTypes.DEFAULT_TYPE, chat_id: int) -> None:
    """React to a linked player's message with an emoji reflecting their current rank tier."""
    user = update.effective_user
    link = get_linked_player_for(chat_id, user.id)
    if not link:
        return
    try:
        db = get_db()
        doc = db.collection(WEBSITE_PLAYERS_COL).document(link["player_id"]).get()
        if not doc.exists:
            return
        _icon, label, _score = compute_player_rank(doc.to_dict() or {})
        emoji = RANK_REACTION_EMOJI.get(label, "👍")
        from telegram import ReactionTypeEmoji
        await context.bot.set_message_reaction(
            chat_id, update.message.message_id, reaction=[ReactionTypeEmoji(emoji)]
        )
    except Exception:
        pass  # purely cosmetic — never let a reaction failure affect anything else


async def player_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Anyone: a player's full card. Usage: /player Player Name"""
    name_arg = update.message.text.partition(" ")[2].strip()
    if not name_arg:
        await update.message.reply_text("Usage: /player Player Name")
        return

    players = fetch_league_players()
    key = name_arg.lower()
    player = next((p for p in players if (p.get("name") or "").strip().lower() == key), None)
    if not player:
        await update.message.reply_text("Couldn't find that player. Check /squad Team Name for exact names.")
        return

    team_names = get_league_team_names()
    icon, label, score = compute_player_rank(player)
    market_value = player.get("marketValue", 0) or 0

    await update.message.reply_text(
        f"<b>{icon} {html.escape(player.get('name', '?'))}</b> — {html.escape(team_names.get(player.get('teamId'), '?'))}\n\n"
        f"⚽ Goals: {player.get('goals', 0)}\n"
        f"🅰️ Assists: {player.get('assists', 0)}\n"
        f"🎽 Appearances: {player.get('appearances', 0)}\n"
        f"⭐ Rating: {player.get('rating', '-')}\n"
        f"💰 Market value: {market_value:,}\n\n"
        f"{icon} <b>{label}</b> (rank score {score})",
        parse_mode=ParseMode.HTML,
    )


async def form_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Anyone: a team's last 5 results. Usage: /form Team Name"""
    team_arg = update.message.text.partition(" ")[2].strip()
    if not team_arg:
        await update.message.reply_text("Usage: /form Team Name")
        return
    website_team = find_website_team_by_name(team_arg)
    if not website_team:
        await update.message.reply_text(f"No website team named '{team_arg}' found.")
        return

    db = get_db()
    entry_doc = db.collection(WEBSITE_TEAM_ENTRIES_COL).document(f"{website_team['id']}_{LEAGUE_ID}").get()
    stats = entry_doc.to_dict() if (entry_doc and entry_doc.exists) else website_team
    form = stats.get("form") or []
    if not form:
        await update.message.reply_text(f"No results yet for {website_team.get('name', team_arg)}.")
        return

    icons = {"W": "🟢W", "D": "🟡D", "L": "🔴L"}
    form_str = " ".join(icons.get(r, r) for r in form[-5:])
    await update.message.reply_text(
        f"<b>📈 {html.escape(website_team.get('name', team_arg))} — Last {len(form[-5:])}</b>\n\n{form_str}",
        parse_mode=ParseMode.HTML,
    )


async def budget_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Anyone: a team's budget, or every team's budget ranked if no name is given."""
    team_arg = update.message.text.partition(" ")[2].strip()
    db = get_db()
    ...
    if team_arg:
        ...
        if not website_team:
            await update.message.reply_text(f"No website team named '{team_arg}' found.")
            return
        budget = website_team.get("budget", 0) or 0
        await update.message.reply_text(
            f"💰 <b>{html.escape(website_team.get('name', team_arg))}</b>: {budget:,}", parse_mode=ParseMode.HTML
        )
        return

    rows = []
    for tid in get_league_team_ids():
        doc = db.collection(WEBSITE_TEAMS_COL).document(tid).get()
        team = doc.to_dict() if doc.exists else {}
        rows.append((team.get("name", "?"), team.get("budget", 0) or 0))
    if not rows:
        await update.message.reply_text("No teams found for this league.")
        return
    rows.sort(key=lambda r: r[1], reverse=True)
    lines = [f"{i+1}. {html.escape(name)} — {budget:,}" for i, (name, budget) in enumerate(rows)]
    await update.message.reply_text("<b>💰 Team Budgets</b>\n\n" + "\n".join(lines), parse_mode=ParseMode.HTML)


async def squad_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Anyone: view a team's real website squad and their stats. Usage: /squad Team Name"""
    team_arg = update.message.text.partition(" ")[2].strip()
    if not team_arg:
        await update.message.reply_text("Usage: /squad Team Name")
        return
    chat_id = update.effective_chat.id
    tg_team = find_registered_team(chat_id, team_arg) if update.effective_chat.type != "private" else team_arg
    website_team = find_website_team_by_name(tg_team or team_arg)
    if not website_team:
        await update.message.reply_text(f"No website team named '{team_arg}' found.")
        return
    squad = fetch_website_squad(website_team["id"])
    if not squad:
        await update.message.reply_text(f"No players registered for {website_team.get('name', team_arg)}.")
        return
    lines = [
        f"{html.escape(p.get('name', '?'))} — {p.get('goals', 0)}G {p.get('assists', 0)}A ({p.get('appearances', 0)} apps)"
        for p in squad
    ]
    await update.message.reply_text(
        f"<b>👥 {html.escape(website_team.get('name', team_arg))} Squad</b>\n\n" + "\n".join(lines),
        parse_mode=ParseMode.HTML,
    )


async def playervs_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Anyone: side-by-side stat comparison between two players. Usage: /playervs Player A, Player B"""
    text = update.message.text.partition(" ")[2]
    if "," not in text:
        await update.message.reply_text("Usage: /playervs Player A, Player B")
        return
    a_name, b_name = (p.strip() for p in text.split(",", 1))

    players = fetch_league_players()

    def find_player(name):
        key = name.strip().lower()
        return next((p for p in players if (p.get("name") or "").strip().lower() == key), None)

    pa, pb = find_player(a_name), find_player(b_name)
    if not pa or not pb:
        await update.message.reply_text("Couldn't find one or both players. Check /squad Team Name for exact names.")
        return

    team_names = get_league_team_names()

    def stat_block(p):
        team = team_names.get(p.get("teamId"), "?")
        market_value = p.get("marketValue", 0) or 0
        return (
            f"{html.escape(p.get('name', '?'))} ({html.escape(team)})\n"
            f"⚽ {p.get('goals', 0)}  🅰️ {p.get('assists', 0)}  🎽 {p.get('appearances', 0)} apps  "
            f"⭐ {p.get('rating', '-')}  💰 {market_value:,}"
        )

    await update.message.reply_text(
        f"<b>⚔️ {html.escape(pa.get('name', '?'))} vs {html.escape(pb.get('name', '?'))}</b>\n\n"
        f"{stat_block(pa)}\n\n{stat_block(pb)}",
        parse_mode=ParseMode.HTML,
    )
...
async def tagall_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin-only: tag every registered member across all teams, and pin the message.
    Usage: /tagall Your announcement text"""
    if update.effective_chat.type == "private":
        await update.message.reply_text("Run this inside the group.")
        return
    if not await is_admin(update, context):
    ...
    chat_id = update.effective_chat.id
    announcement = update.message.text.partition(" ")[2].strip() or "📢 Attention everyone!"

    everyone = {}
    for info in get_teams(chat_id).values():
        everyone.update(info["members"])
    if not everyone:
        await update.message.reply_text("No registered members to tag yet.")
        return

    mentions = ", ".join(tag_mention(uid, name) for uid, name in everyone.items())
    sent = await context.bot.send_message(
        chat_id, f"<b>{html.escape(announcement)}</b>\n\n{mentions}", parse_mode=ParseMode.HTML
    )
    try:
        await context.bot.pin_chat_message(chat_id, sent.message_id)
    except BadRequest:
        await update.message.reply_text(
            "(Sent, but couldn't pin it — make sure I have the \"Pin Messages\" admin permission.)"
        )


async def pin_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin-only: reply to any message with /pin to pin it."""
    if update.effective_chat.type == "private":
        await update.message.reply_text("Run this inside the group.")
        return
    if not await is_admin(update, context):
        ...
        return
    if not update.message.reply_to_message:
        await update.message.reply_text("Reply to the message you want pinned with /pin.")
        return
    try:
        await context.bot.pin_chat_message(update.effective_chat.id, update.message.reply_to_message.message_id)
    except BadRequest:
        await update.message.reply_text("Couldn't pin — make sure I have the \"Pin Messages\" admin permission.")


async def h2h_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Anyone: head-to-head record between two teams. Usage: /h2h Team A, Team B"""
    text = update.message.text.partition(" ")[2]
    if "," not in text:
        await update.message.reply_text("Usage: /h2h Team A, Team B")
        return
    a_name, b_name = (p.strip() for p in text.split(",", 1))
    team_a = find_website_team_by_name(a_name)
    team_b = find_website_team_by_name(b_name)
    if not team_a or not team_b:
        await update.message.reply_text("Couldn't find one or both teams on the website.")
        return
    ...
    played = [
        m for m in matches
        if m.get("status") == "played"
        and {m.get("homeTeamId"), m.get("awayTeamId")} == {team_a["id"], team_b["id"]}
    ]
    if not played:
        await update.message.reply_text(f"No past meetings found between {a_name} and {b_name}.")
        return

    a_wins = b_wins = draws = 0
    lines = []
    for m in sorted(played, key=lambda x: x.get("matchday", 0)):
        home, away = m.get("homeTeamName", "?"), m.get("awayTeamName", "?")
        hs, aws = m.get("homeScore", 0), m.get("awayScore", 0)
        lines.append(f"MD{m.get('matchday', '?')}: {html.escape(home)} {hs} - {aws} {html.escape(away)}")
        if hs == aws:
            draws += 1
        elif (hs > aws) == (m.get("homeTeamId") == team_a["id"]):
            a_wins += 1
        else:
            b_wins += 1

    summary = f"{team_a.get('name')} {a_wins} - {b_wins} {team_b.get('name')} ({draws} draw(s))"
    await update.message.reply_text(
        f"<b>⚔️ Head to Head: {html.escape(summary)}</b>\n\n" + "\n".join(lines), parse_mode=ParseMode.HTML
    )
...
async def maybe_auto_join_by_member_tag(update: Update, context: ContextTypes.DEFAULT_TYPE, chat_id: int) -> None:
    """If someone's Telegram Member Tag mentions exactly one registered team, auto-join them to
    it — as long as they aren't on a team already and the match isn't ambiguous. Checks the
    March-2026 "tag" field (available to any member, if the group's admin turned on Group
    Settings > Permissions > Edit Own Tags) as well as the older admin-only custom_title."""
    user = update.effective_user
    if find_team_of_user(chat_id, user.id) is not None:
        return  # already on a team — never override an existing assignment

    try:
        member = await context.bot.get_chat_member(chat_id, user.id)
    except Exception:
        return
    tag = getattr(member, "tag", None) or getattr(member, "custom_title", None)
    if not tag:
        return  # no Member Tag set
    tag_lower = tag.strip().lower()

    chat_teams = get_teams(chat_id)
    matches = [
        name for name in chat_teams
        if re.search(rf"\b{re.escape(name.lower())}\b", tag_lower)
    ]
    if len(matches) != 1:
        return  # no team mentioned, or ambiguous between multiple — ignore either way

    team_name = matches[0]
    chat_teams[team_name]["members"][user.id] = user.first_name
    save_teams()
    await update.message.reply_text(
        f"👋 Spotted '{team_name}' in your Member Tag — added you to team {team_name}!"
    )
...
async def handle_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if sender and sender.username:
        username_cache.setdefault(chat_id, {})[sender.username.lower()] = (sender.id, sender.first_name)
    await maybe_auto_join_by_member_tag(update, context, chat_id)
    await maybe_react_with_rank_badge(update, context, chat_id)
...
def main():
    ...
    app.add_handler(CommandHandler("topscorers", topscorers_cmd))
    app.add_handler(CommandHandler("topassists", topassists_cmd))
    ...
    app.add_handler(CommandHandler("h2h", h2h_cmd))
    app.add_handler(CommandHandler("playervs", playervs_cmd))
    ...
    app.add_handler(CallbackQueryHandler(resmd_pick_callback, pattern="^resmd:"))
    app.add_handler(CallbackQueryHandler(resmd_back_callback, pattern="^resmdback$"))
