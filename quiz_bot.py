...
def format_leaderboard(chat_id: int) -> str:
    ...
    return "\n".join(f"{entry['name']}: {entry['score']} pt(s)" for entry in ordered)
...
async def startquiz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not state.game_active:
        ...
        if arg_text:
            ...
            playing_desc = ", ".join(sorted(matched))
            await update.message.reply_text(
                f"🎮 New team match started! Teams playing: {playing_desc}. "
                f"First to be marked correct on each question scores a point. {GAME_LENGTH} questions per game."
            )
        else:
            # Individual leaderboard mode — reset everyone's score for this new game.
            for entry in get_leaderboard(chat_id).values():
                entry["score"] = 0
            save_leaderboard()
            ...
            await update.message.reply_text(
                "🎮 New open game started — anyone in the group can answer! An admin picks the correct "
                f"answerer from the list after each question; the faster you're correct, the more points "
                f"(up to {MAX_POINTS_PER_QUESTION}). {GAME_LENGTH} questions per game."
            )
...
async def post_question_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    scoring_note = (
        "First correct answer scores a point for that member's team."
        if is_team_mode(state)
        else f"The faster you're marked correct, the more points you earn (up to {MAX_POINTS_PER_QUESTION})."
    )
    await context.bot.send_message(
        chat_id,
        f"❓ Question {state.questions_asked_this_game}/{GAME_LENGTH}: {question}\n\n"
        f"⏱ {ANSWER_WINDOW_SECONDS} seconds! Answer in chat — after time's up I'll list everyone's "
        f"answers and an admin picks the winner by replying \"Correct\" to their message. {scoring_note}",
    )
...
async def resolve_correct(update: Update, context: ContextTypes.DEFAULT_TYPE, state: Optional[QuizState], chat_id: int):
    ...
    if is_team_mode(state):
        ...
        # Individual leaderboard mode — points scale with how fast the replied-to message arrived.
        ...
        await update.message.reply_text(
            f"✅ Correct! {target_user.first_name} answered in {elapsed}s and earns {points} point(s)!\n\n"
...
async def close_window(context: ContextTypes.DEFAULT_TYPE):
    ...
    if not state.submissions:
        await context.bot.send_message(
            ...
            "Use /skip to move on, or reply \"Correct\" to a late answer to still credit it.",
        )
        return

    lines = [f"{n}. {name}: {ans}  ({t}s)" for n, name, ans, t, _mid in state.submissions]
    await context.bot.send_message(
        chat_id,
        "⏰ Time's up! Here's who answered, in order:\n"
        + "\n".join(lines)
        + "\n\nAdmin: reply \"Correct\" to the right message to award the point, "
        "or /skip if no one got it.",
    )
...
async def skip_question(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"⏭ Skipped. Correct answer was: {state.last_answer}\n\n"
        f"📊 {label}:\n{standings}"
    )
    await advance_or_end_game(chat_id, context)
